...
// gate fails.
//
// RETIRED ATTEMPT 1: preserved for forensic traceability only. It automatically
// rolled back before surface evaluation because it mishandled the CLI result
// shape. Do not run it; the corrected, successful transaction is recorded in
// promote_lari_public_surface_uncertainty_candidate_attempt2.js and its
...
const INCUMBENT_REGISTRY_HASH = '7caadb3c2f05d82779ccea1eb36b7087f0533f115e44a530e7d5f1a4a06aa792';
...
const OUT = path.join(ROOT, 'consolidation', 'public-surface-uncertainty-production-promotion-20260906');
...
function preflight() {
  ...
  [CANDIDATE, REHEARSAL, registry.currentModelPath, registry.registryPath].forEach(file => {
  ...
  const gates = {
    ...
    rehearsalIncumbentExact: rehearsal.incumbent?.sha256 === INCUMBENT_HASH
      && rehearsal.incumbent?.registrySha256 === INCUMBENT_REGISTRY_HASH,
