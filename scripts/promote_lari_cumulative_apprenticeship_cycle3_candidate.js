...
// One-shot real-production promotion for the immutable cumulative-
// apprenticeship Cycle 3 candidate. It deliberately uses the existing registry
...
const INCUMBENT_HASH = '034cf64f73dd78b4da1f9f6b4a30e4a3a59f5d81a136d1f6c6de49558ca69a22';
const INCUMBENT_REGISTRY_HASH = '90d75ac3fa18a7c888d2a8629e74a5c44beb789884823bfe7207fe55954600c5';
const REHEARSAL_INCUMBENT_REGISTRY_HASH = INCUMBENT_REGISTRY_HASH;
const CANDIDATE_PARENT_HASH = '11bdeeacf3a605cf5959d8ddc6cc980848c98e2ad1025c0034c7c55e7d16cb77';
const REHEARSAL_HASH = '2b59b57b1dbceddf4f211e46538d94d83d0894c689a8807b2d12948ba97548c0';
...
const EXPECTED_ADDITIONAL_RECORD_IDS = [
  ...
  'lari.learned.repair.capability_gap.f7f5077e98f58d684b0e',
  'lari.learned.operator.neurogenesis.c35818debd7272fc62c9',
  'lari.learned.repair.capability_gap.b85dd57c6e5e4587a925',
  'lari.learned.operator.neurogenesis.499acbcdd894611c290c',
  'lari.learned.repair.capability_gap.9f9488468ea052e20b44',
  'lari.learned.procedure.coordinated.f5a61e1f',
  'lari.learned.operator.neurogenesis.5e479304612371ddb958',
  'lari.learned.repair.capability_gap.10d4309d6f11b20bfb06',
...
const CANDIDATE = path.join(
  ROOT,
  'consolidation',
  'sealed-cumulative-apprenticeship-cycle3-20260913',
  'candidates', `${CANDIDATE_HASH}.json`
);
const REHEARSAL = path.join(
  ROOT,
  'consolidation',
  'sealed-cumulative-apprenticeship-cycle3-promotion-rehearsal-20260913',
...
const OUT = path.join(ROOT, 'consolidation', 'sealed-cumulative-apprenticeship-cycle3-production-promotion-20260913');
...
const USER_SCOPE = 'cumulative-apprenticeship-production-promotion';
...
function preflight() {
  ...
  [CANDIDATE, REHEARSAL, QUALIFICATION, registry.currentModelPath, registry.registryPath].forEach(file => {
  ...
  const gates = {
    ...
    candidateIsHashLocked: shaFile(CANDIDATE) === CANDIDATE_HASH
      && rehearsal.candidate?.sha256 === CANDIDATE_HASH
    ...
    sealedQualificationPassed: qualification.passed === true
      && qualification.candidate?.sha256 === CANDIDATE_HASH
      && Object.values(qualification.gates || {}).every(value => value === true),
    ...
    omittedCompiledEntriesHaveTypedBackings: omittedCompiled.every(item => item.backingTypedRecordPresent),
...
function automaticRollback(error, backups) {
  ...
  if (before.active === CANDIDATE_HASH && fs.existsSync(ACTIVE_BACKUP)) {
    const restored = registry.promoteLariModel(ACTIVE_BACKUP, {
      stage: 'cumulative-apprenticeship-cycle3-production-promotion',
      transaction: 'automatic-rollback-cumulative-apprenticeship-cycle3',
      rollback: true,
  ...
  if (!fs.existsSync(FAILURE)) {
    writeExclusive(FAILURE, {
      schemaVersion: 1,
      kind: 'lari.cumulative-apprenticeship-cycle3.production-promotion.failure',
...
async function main() {
  ...
  writeExclusive(PREFLIGHT, {
    schemaVersion: 1,
    kind: 'lari.cumulative-apprenticeship-cycle3.production-preflight',
    createdAt: new Date().toISOString(),
    ...
    qualification: fileInfo(QUALIFICATION),
    candidateLineage: {
  ...
  try {
    ...
    const validation = {
      schemaVersion: 1,
      kind: 'lari.cumulative-apprenticeship-cycle3.production-post-promotion-validation',
      createdAt: new Date().toISOString(),
      passed: true,
      ...
      limitations: [
        ...
        'Cycle 3 proves bounded cumulative apprenticeship over eight sealed tasks; it does not prove arbitrary coding or general intelligence.'
      ],
      externalModelCalls: 0
    };
    ...
    const manifest = {
      schemaVersion: 1,
      kind: 'lari.cumulative-apprenticeship-cycle3.production-promotion',
      createdAt: new Date().toISOString(),
      passed: true,
