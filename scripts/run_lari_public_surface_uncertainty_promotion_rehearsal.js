...
// Isolated promotion rehearsal for the immutable public-surface uncertainty
// candidate. This is deliberately a test harness around the existing atomic
...
const CANDIDATE = path.join(ROOT, 'consolidation', 'public-surface-uncertainty-convergence-attempt4-20260906', 'candidate-beb2db6daa870dc134781ff9135247b942d3c578951b6fe90eacd03fbe9fca15.json');
const CANDIDATE_HASH = 'beb2db6daa870dc134781ff9135247b942d3c578951b6fe90eacd03fbe9fca15';
const PARENT_HASH = 'ac842c3803ca7288b84555780a9f3ddeb7f0f1ec67ad077a91c25ffd1b7b8511';
...
const OUT = path.join(ROOT, 'consolidation', 'public-surface-uncertainty-promotion-rehearsal-attempt2-20260906');
...
const ISOLATED = path.join(ROOT, 'consolidation', 'stage-3-rehearsal', 'public-surface-uncertainty-promotion-rehearsal-attempt2-20260906');
const USER_SCOPE = 'public-surface-rehearsal';
...
async function main() {
  ...
  for (const input of [ACTIVE, REGISTRY, LEGACY_ROOT, PARENT, CANDIDATE]) {
  ...
  const candidateProvenance = {
    ...
    onlyExpectedLearnedRecordAdded: addedRecords.length === 1 && addedRecords[0] === TYPED_RECORD_ID,
  ...
  const evidence = {
    schemaVersion: 1,
    kind: 'lari.public-surface-uncertainty.promotion-rehearsal',
    createdAt: new Date().toISOString(),
    rehearsalOnly: true,
    realPromotionPerformed: false,
    candidate: { path: rel(CANDIDATE), sha256: CANDIDATE_HASH, parentHash: PARENT_HASH, typedRecordId: TYPED_RECORD_ID },
    ...
    limitations: [
      'The retained cobalt-orbit knowledge source remains a local fixture; this rehearses lifecycle mechanics rather than arbitrary web-source quality.',
  ...
  writeExclusive(path.join(OUT, 'PROMOTION_REHEARSAL_REPORT.md'), [
    '# Public-surface uncertainty promotion rehearsal', '',
